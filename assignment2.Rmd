...
Egoalt8 <- read_tsv("C:/Users/User/Documents/GitHub/dataan3/UKDA-6614-tab/tab/ukhls_w8/h_egoalt.tab")
Stable <- read_tsv("C:/Users/User/Documents/GitHub/dataan3/UKDA-6614-tab/tab/ukhls_wx/xwavedat.tab")
Hh8 <- read_tsv("C:/Users/User/Documents/GitHub/dataan3/UKDA-6614-tab/tab/ukhls_w8/h_hhresp.tab")
```
...
```{r}
Partners8 <- Egoalt8 %>%
        filter(h_relationship_dv=="1"|Egoalt8$h_relationship_dv=="2") %>%
...
```{r}
JoinedEthn <- JoinedEthn %>%
        full_join(Stable2, c("apidp"="pidp"), na.rm=TRUE)
```
...
```{r}
JoinedEthn <- JoinedEthn %>%
        full_join(Hh8, by= "h_hidp", na.rm=TRUE)%>%
        select(h_hidp,pidp, egoRacel_dv,alterRacel_dv, h_nkids_dv)
...
```{r}
JoinedEthn <- JoinedEthn %>%
        mutate(egoRacel_dv = ifelse(egoRacel_dv==1, "White British",
...
```{r}
TableRaceKids<- JoinedEthn %>%
       select(egoRacel_dv, h_nkids_dv)%>%
        group_by(egoRacel_dv)%>%
        summarise(
                meankids = mean(h_nkids_dv),
                mediankids= median(h_nkids_dv)
        )
```
4) Write a short interpretation of your results. What could affect your findings?

The results of the table show that there are large differences between the average and median number of children in White British, Indian and Pakistani families. 
The mean number of children in White British families is 0.6, with a median number of 0. This is in stark contrast to Pakistani families, with a mean number of 1.8 and median number of 2. Indian families sit between these figures, with a mean and median number of 1.
These figures do not take into account the ages of the couples and so this may skew the data and results- there may be more young White British couples who are too young to settle and have a family. 